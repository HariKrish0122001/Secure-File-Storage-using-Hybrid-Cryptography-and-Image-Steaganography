...
class RefreshTokenCredential(Credential):
    ...
    def token(self) -> str:
        if not self._cached_token or self._cached_token.get('expires_in') <= int(round(time())):
            from zcatalyst._http_client import HttpClient  # pylint: disable=cyclic-import,import-outside-toplevel
            ...
            try:
                ...
                raise CatalystAuthenticationError('Error while fetching access token') from None
...
class CatalystCredential(Credential):
    def __init__(
        ...
        self._user_type: Union[AccessTokenCredential, TicketCredential, CookieCredential] \
        ...
        # Admin Credentials
        if self._admin_cred_type.__eq__(CredentialType.ticket):
            self._admin_cred = TicketCredential({
                APIConstants.TICKET: self._admin_token
            })
        elif self._admin_cred_type.__eq__(CredentialType.token):
        ...
        if self._user_cred_type.__eq__(CredentialType.token):
            self._user_cred = AccessTokenCredential({
                APIConstants.ACCESS_TOKEN: self._user_token
            })
        elif self._user_cred_type.__eq__(CredentialType.ticket):
