            ...
            if not connector.get(key):
                raise CatalystConnectorError(
                    ...
                    (f"Either the key '{key}' is missing or value "
                     "provided for the {key} is None in {connector_name} dict")
                )
...
def instance(properties: Union[str, Dict[str, Dict[str, str]]], app=None) -> ConnectionService:
    return _utils.get_ensured_app_service(
        app, _CONNECTION, ConnectionService, override=True, properties=properties
    )
