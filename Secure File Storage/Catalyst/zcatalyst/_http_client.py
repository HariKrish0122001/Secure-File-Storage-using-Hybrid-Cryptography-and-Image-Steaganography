        ...
        if self._status_code not in range(200, 300):
            ...
            raise CatalystAPIError(
                ...
                (f'Request failed with status {self._status_code} and '
                 'response data : {self.response_json}')
            )
    ...
    def request(
        ...
        if self._app is not None:
            if not isinstance(self._app, CatalystApp):
                raise ValueError(
                    'Invalid app provide to make requests. App must be an instance of CatalystApp'
                )

            headers[PROJECT_KEY] = str(self._app.config.get(PROJECT_KEY))
            ...
            user = self._app.credential.current_user_type()
